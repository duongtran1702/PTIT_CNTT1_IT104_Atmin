...
const sqrt = (base: number, root: number): number | string =>
    root === 0 ? 'Root cannot be zero' : Math.pow(base, 1 / root);
const factorial = (n: number): number | string => {
    if (n < 0 || !Number.isInteger(n)) return 'Invalid input for factorial';
    let result = 1;
...
const displayResult = (a: string | number, b: string | number | null, op: string, result: number | string) => {
...
// Lấy input từ form
const getInputs = (): [number | null, number | null] => {
...
(document.getElementById('add') as HTMLButtonElement).onclick = () => {
    const [a, b] = getInputs();
    if (a === null || b === null) return displayResult('', '', '', 'Invalid input');
...
(document.getElementById('factorial') as HTMLButtonElement).onclick = () => {
    ...
    if (a === null) return displayResult('', '', '', 'Invalid input');
